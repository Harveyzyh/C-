...
{
    public partial class 录入销退单 : Form
    {
        #region 公用变量设定
        ...
        private string TypeID = "2401";
        private string PositionID = "401";
        ...
        private void 销货仓库L_MouseClick(object sender, MouseEventArgs e)
        {
            ...
            Title = "仓库编号";
            //Title = "仓库编号|仓库名称";
            录入销退单_获取单据信息 frm = new 录入销退单_获取单据信息();
        ...
        private string GetFlowID(string flowId = null)
        {
            ...
            {
                if (mssql.SQLselect(connStr, string.Format("SELECT RTRIM(XTXA005) FROM dbo.XT_LYXA WHERE XTXA005 = '{0}' ", flowId)) == null)
                {
        ...
        {
            string sqlstr = @"SELECT RTRIM(TH004), RTRIM(MB002), RTRIM(MB003), RTRIM(TG004), SL FROM V_COPTH_SL_WG 
                                INNER JOIN INVMB ON MB001 = TH004 
                                WHERE TH004 = '{0}' AND TG004 = '{1}' ";
        ...
        private void Insert()
        {
            string sql = "INSERT INTO XT_LYXA "
                        + "(COMPANY, CREATOR, USR_GROUP, CREATE_DATE, FLAG, XTXA001, XTXA002, XTXA003, "
                        + "XTXA004, XTXA005, XTXA007, XTXA008, XTXA009, "
                        + "XTXA011, XTXA012, XTXA013, XTXA014, XTXA015, ID) "
            ...
            string tell = "\n\r处理流水号为：" + flowId;
            string sqlstrEXEC = @"EXEC dbo.P_COPTI_CREATE_WORK_WG '{0}'";
