...
{
    public partial class 录入进货单_联友分公司 : Form
    {

        #region 局部变量设定
        Mssql mssql = new Mssql();
        string connF_YF = "Server = 183.56.211.111,8087; initial catalog = COMFORT3; user id = sa; password=DGlsdnkj168;Connect Timeout = 5";
        ...
        #region 窗口初始化
        public 录入进货单_联友分公司(string text = "")
        {
            InitializeComponent();
        ...
        {
            string sqlstr_F = @"SELECT  MIN(TH003) TH003, RTRIM(TH004) 品号, '' 品名, '' 规格, CAST(SUM(TH008) AS FLOAT) SL, 'A0263' 供应商编号 FROM COPTG 
            ...
            string sqlstr = @"SELECT RTRIM(MB002), RTRIM(MB003) FROM dbo.INVMB WHERE MB001 = '{0}' ";

            DataTable dt = mssql.SQLselect(connF_YF, string.Format(sqlstr_F, 条码T.Text));

            if (dt != null)
            {
                for (int index = 0; index < dt.Rows.Count; index++)
                {
                    DataTable dt2 = mssql.SQLselect(connYF, string.Format(sqlstr, dt.Rows[index][1].ToString()));
                    ...
                    if (dt2 != null)
                    {
                        this.DataGridView_List.Rows[row].Cells[2].Value = dt2.Rows[0][0].ToString();
                        this.DataGridView_List.Rows[row].Cells[3].Value = dt2.Rows[0][1].ToString();
                        this.DataGridView_List.Rows[row].DefaultCellStyle.ForeColor = Color.Black;
                    }
                    else
                    {
                        this.DataGridView_List.Rows[row].Cells[2].Value = "品号信息异常";
                        this.DataGridView_List.Rows[row].DefaultCellStyle.ForeColor = Color.Red;
                    }
                }
            }
            else
            {
                MessageBox.Show("该销货单不存在", "错误");
                送货单号T.Text = "";
            }
            GetIndex();
            ShowUI();
