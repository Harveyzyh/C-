...
namespace 联友生产辅助工具.生管排程
{
    public partial class 生管排程导入导出部门选择 : Form
    {
        ...
        private void Init()
        {
            if(Mode == "导入")
            {
                string sqlstr = "SELECT Dpt FROM dbo.SC_PLAN_DPT_TYPE WHERE Valid = 1 AND Type = 'In' ORDER BY [Index]";
            ...
            {
                string sqlstr = "SELECT Dpt FROM dbo.SC_PLAN_DPT_TYPE WHERE Valid = 1 AND Type = 'Out' ORDER BY [Index]";
                DataTable dt = mssql.SQLselect(connWG, sqlstr);
                if (dt != null)
                {
                    for (int rowIdx = 0; rowIdx < dt.Rows.Count; rowIdx++)
