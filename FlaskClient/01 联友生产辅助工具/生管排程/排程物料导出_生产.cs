    ...
    public partial class 排程物料导出_生产 : Form
    {
        ...
        private DataTable GetShowDt()
        {
            string sqlStr = @"SELECT SCPLAN.SC003 排程日期, SCPLAN.SC023 生产车间, SCPLAN.SC001 生产单号, 
                                ...
                                RTRIM(MOCTB.TB003) 物料品号, RTRIM(MOCTB.TB012) 材料品名, RTRIM(MOCTB.TB013) 材料规格, RTRIM(MOCTB.TB006) 工艺, CAST(SUM(MOCTB.TB004) AS FLOAT) 需领料量, 
                                RTRIM(CMSMW.MW002) 工艺名称, CONVERT(VARCHAR(100), CMSMW.MW003) 组别, 
                                RTRIM((CASE WHEN INVMB2.MB002 IN('气压棒', '中管气压棒') AND COPTD.UDF09 IS NOT NULL AND COPTD.UDF09 != '' THEN PURMA3.MA001 ELSE PURMA1.MA001 END)) 批号, 
                                RTRIM((CASE WHEN INVMB2.MB002 IN('气压棒', '中管气压棒') AND COPTD.UDF09 IS NOT NULL AND COPTD.UDF09 != '' THEN PURMA3.MA002 ELSE PURMA1.MA002 END)) 批号说明, 
                                RTRIM(INVMB2.UDF02) 货位号, '' AS 备料员, RTRIM(SCPLAN.SC002) 订单类别, 
								RTRIM((CASE WHEN MOCTA.TAU01=INVMB2.MB001 AND SUBSTRING(MOCTA.TAU02, 1, CHARINDEX('*', MOCTA.TAU02)-1)>500 THEN SCPLAN.SC002 ELSE '' END)) 纸箱类型, 
								RTRIM((CASE WHEN MOCTA.TAU01 = INVMB2.MB001 THEN MOCTA.TAU02 ELSE '' END)) 外包装尺寸 
                                FROM WG_DB.dbo.SC_PLAN(NOLOCK) AS SCPLAN 
            ...
            sqlStr += @"AND MOCTB.TB011 IN ('1', '2', '3', '5') 
                        ...
                        GROUP BY SCPLAN.SC003, SCPLAN.SC023, SCPLAN.SC001, 
                            ...
                            RTRIM(INVMB2.UDF02), SCPLAN.SC002, (CASE WHEN MOCTA.TAU01=INVMB2.MB001 AND SUBSTRING(MOCTA.TAU02, 1, CHARINDEX('*', MOCTA.TAU02)-1)>500 THEN SCPLAN.SC002 ELSE '' END), 
