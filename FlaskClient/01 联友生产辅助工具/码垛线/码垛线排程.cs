...
namespace HarveyZ.生管码垛线
{
    public partial class 码垛线排程导入 : Form
    {
        ...
        private void btnOutput_Click(object sender, EventArgs e)
        {
            ...
            excelObj.dataSet.Tables[0].TableName = "生产日报表导出";
        ...
        {
            string show_slqStr = " SELECT DISTINCT "
                                         + " SC003 AS 上线日期, "
                                         + " SC001 AS 订单号, "
            ...
            if (show)
            {
                show_slqStr += " AND SC003 >= CONVERT(VARCHAR(20), GETDATE(), 112)";
            }
            show_slqStr += " ORDER BY SC003, (CASE SC026 WHEN 'Y' THEN '是' ELSE '' END ) DESC, SC001 ";

            if (showDt != null)
            {
                showDt.Clear();
            }
            showDt = mssql.SQLselect(connMD, show_slqStr);
        ...
        private static class MdHandelClass
        {
            ...
            {
                string slqStr = @"UPDATE dbo.SCHEDULE SET SC038 = 'n' WHERE SC003 >= CONVERT(VARCHAR(20), DATEADD(DAY, -1, GETDATE()), 112) AND SC039 != 'Y'";
                mssql.SQLexcute(connMD, slqStr);
            }
            ...
            public static void GetBoxCode()
            {
                ...
                string slqStrBoxName = "SELECT BoxName, Spec from BoxNameCode where Valid = 1 order by Spec";

                string slqStrList = @"SELECT SC001 FROM dbo.SCHEDULE(NOLOCK) WHERE 1=1 AND SC039 = 'N' AND SC038 = 'y' AND SC039 != 'Y' ORDER BY SC003, SC001";

                string slqStrMocta = @"SELECT TA001 FROM dbo.MOCTA(NOLOCK) INNER JOIN dbo.MOCTB(NOLOCK) ON TA001 = TB001 AND TA002 = TB002 WHERE RTRIM(TA026)+'-'+RTRIM(TA027)+'-'+RTRIM(TA028) = '{0}'";

                string slqStrMoctb = @"SELECT TOP 1 ISNULL(MBU01, '') FROM dbo.MOCTA(NOLOCK) 
					                    INNER JOIN dbo.MOCTB(NOLOCK) ON TA001 = TB001 AND TA002 = TB002 AND TB004 > 0
                ...
                string slqStrUdtSc040 = @"UPDATE ROBOT_TEST.dbo.SCHEDULE SET SC040 = '{1}', SC038 = 'Y' WHERE SC001 = '{0}' ";

                string slqStrUdtSc036 = @"UPDATE ROBOT_TEST.dbo.SCHEDULE SET SC036 = ISNULL(BoxCode, 'NULL') FROM dbo.SCHEDULE LEFT JOIN ROBOT_TEST.dbo.BoxSizeCode ON BoxSize = SC040 WHERE (SC036 = 'NULL' OR SC036 = '' OR SC036 IS NULL) ";

                //构建纸箱名称的sql语句
                DataTable dtBoxName = mssql.SQLselect(connMD, slqStrBoxName);
                if (dtBoxName != null)
                {
                ...
                DataTable dtList = mssql.SQLselect(connMD, slqStrList);
                if (dtList != null)
                {
                    for(int rowIndex =0; rowIndex < dtList.Rows.Count; rowIndex++)
                    {
                        ...
                        if(mssql.SQLexist(connERP, string.Format(slqStrMocta, sc001)))
                        {
                            DataTable dt = mssql.SQLselect(connERP, string.Format(slqStrMoctb, sc001, boxNameStr));
                            if (dt != null)
                            {
                        ...
                        mssql.SQLexcute(connMD, string.Format(slqStrUdtSc040, sc001, sc040));
                    }
                    mssql.SQLexcute(connMD, slqStrUdtSc036);
                }
            }
