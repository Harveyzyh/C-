...
namespace HarveyZ.生管码垛线
{
    public partial class 订单类别编码添加 : Form
    {
        ...
        {
            //string slqStr = @"INSERT INTO SplitTypeCode ( PO_Class, PO_Type, TypeCode) VALUES( '{0}', '{1}', '{2}')";

            //string num = mssql.SQLselect(connStrRobot, " Select MAX(K_ID) +1 FROM SplitTypeCode").Rows[0][0].ToString();

            //mssql.SQLexcute(connStrRobot, string.Format(slqStr, textBox1.Text, textBox2.Text, 码垛线排程导入.Num2Char(num)));
