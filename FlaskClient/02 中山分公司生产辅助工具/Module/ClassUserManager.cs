...
namespace HarveyZ
{
    ...
    public class Client_UserLogin
    {
        ...
        #region 添加用户
        public bool AddUser(string U_ID, string U_PWD, string DPT)
        {
            string sqlstr = @"INSERT INTO WG_DB.dbo.WG_USER (U_ID, U_NAME, U_PWD, DPT, FLAG, TYPE) VALUES('{0}', '{1}', '{2}', '{3}', 'Y', '{4}') ";
            mssql.SQLexcute(conn, string.Format(sqlstr, U_ID, U_PWD, DPT, type));
            return false;
        }
        #endregion
        ...
        public bool ChangePwd(string U_ID, string OldPwd, string NewPwd, out string Msg)
        {
            ...
            string sqlstrUpt = @" UPDATE WG_DB.dbo.WG_USER SET U_PWD = '{}' WHERE U_ID = '{0}' AND TYPE = '{1}'";
            DataTable dt = mssql.SQLselect(conn, string.Format(sqlstrFind, U_ID, type));
            if (dt != null)
            {
                if (dt.Rows[0][0].ToString() == OldPwd)
                {
                    mssql.SQLexcute(conn, string.Format(sqlstrUpt, U_ID, NewPwd, type));
                    return true;
                }
                else
                {
