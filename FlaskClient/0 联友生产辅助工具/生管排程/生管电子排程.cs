    ...
    public partial class 生管电子排程 : Form
    {
        ...
        private bool CheckDt(DataTable dt)
        {
            ...
            for (int rowIndex = 0; rowIndex < rowTotal; rowIndex++)
            {
                ...
                SC023 = dt.Rows[rowIndex][22].ToString();

                if(Normal.GetSubstringCount(SC001, "-") != 2 || SC003 == "" || SC023 == "")
                {
        ...
        private string dt2sqlstr(DataTable dttmp)//数据表转成sql语句
        {
            ...
            string SC003 = "", SC001 = "", SC013 = "", SC014 = "", SC023 = "";
            ...
            for (; row < row_total; row++ )
            {
                ...
                try
                {
                    ...
                    if(WorkTime < SysTime && WorkTime > SysTime + 7)
                    {
                        continue;
                    }
                }
                ...
                else if (dttmp.Rows[row][2].ToString() != "")
                {
                    ...
                    dtRowTmp[0] = SC001;
                    dtRowTmp[1] = SC003;
                    dtRowTmp[2] = dttmp.Rows[row][12].ToString();
                    dtRowTmp[3] = dttmp.Rows[row][13].ToString();
                    dtRowTmp[4] = dttmp.Rows[row][22].ToString();

                    dtInsert.Rows.Add(dtRowTmp);

                    returnstr += SC001 + ":" + SC003 + ":" + SC013 + ":" + SC023 + "; ";


                    if (mssql.SQLexist(Conn_WG_DB, string.Format(@"SELECT SC001 FROM SCHEDULE WHERE SC001 = '{0}'", SC001)))
                    {
                        updatestr = "UPDATE SCHEDULE SET "
                                    ...
                                    + " SC038 = 'N' "
                                    + " WHERE SC001 = '" + SC001 + "' ";
                        mssql.SQLexcute(Conn_WG_DB, updatestr);
                    }
                    else
                    {
                        insertstr = "INSERT INTO SCHEDULE (CREATOR, CREATE_DATE, SC001, SC003) VALUES ( "
                                  ...
                                  + "'" + SC001 + "', '" + SC003 + "')";
                        mssql.SQLexcute(Conn_WG_DB, insertstr);
                    }
                }
                else continue;
            }

            if(dtInsert != null) SqlInsertWork(WorkDate: (WorkTime + 1).ToString(), dt:dtInsert);
        ...
        {
            string sqlstrDel = @"DELETE FROM SC_PLAN WHERE SC001 >= '{0}' ";
            string sqlstrInsert = @"INSERT INTO SCHEDULE (CREATOR, CREATE_DATE, SC001, SC003, SC013, SC014, SC023) 
                                    VALUES ('{0}', (CONVERT(VARCHAR(20), GETDATE(), 112) + REPLACE(CONVERT(VARCHAR(20), GETDATE(), 24), ':', '')), 
                                    '{1}', '{2}', '{3}', '{4}'";
            mssql.SQLexcute(Conn_WG_DB, string.Format(sqlstrDel, WorkDate));
            for(int rowIndex = 0; rowIndex < dt.Rows.Count; rowIndex++)
            {
                mssql.SQLexcute(Conn_WG_DB, string.Format(sqlstrInsert, dt.Rows[rowIndex][0], dt.Rows[rowIndex][1], dt.Rows[rowIndex][2], 
                    dt.Rows[rowIndex][3], dt.Rows[rowIndex][4]));
            }
        }
        ...
        private void getdatatable(bool show) //数据库资料显示到界面
        {
            ...
            show_datatable = mssql.SQLselect(Conn_WG_DB, show_sqlstr);
