...
{
    public partial class 测试_码垛线排程导入 : Form
    {
        #region 静态数据设置
        ...
        private string dt2sqlstr(DataTable dttmp)//数据表转成sql语句
        {
            ...
            for (; row < row_total; row++ )
            {
                ...
                else if (dttmp.Rows[row][2].ToString() != "")
                {
                    ...
                    SC002 = dttmp.Rows[row][1].ToString();
                    SC013 = dttmp.Rows[row][3].ToString();
                    SC017 = dttmp.Rows[row][4].ToString();
                    SC036 = dttmp.Rows[row][5].ToString();
                    SC037 = dttmp.Rows[row][6].ToString();
                    ...
                    {
                        insertstr = "INSERT INTO SCHEDULE (CREATOR, CREATE_DATE, SC001, SC002, SC003, SC013, SC017, SC036, SC037, SC039) VALUES ( "
                                  + "'" + FormLogin.Login_Uid + "', (CONVERT(VARCHAR(20), GETDATE(), 112) + REPLACE(CONVERT(VARCHAR(20), GETDATE(), 24), ':', '')), "
                                  + "'" + SC001 + "', '" + SC002 + "', '" + SC003 + "', '" + SC013 + "', '" + SC017
                                  + "', '" + SC036 + "', '" + SC037 + "', 'Y' )";
        ...
        private void getdatatable(bool show) //数据库资料显示到界面
        {
            string show_sqlstr = " SELECT DISTINCT "
                                         ...
                                         + " (CASE SC033 WHEN '0' THEN '未完成' WHEN '1' THEN '已完成' END) AS 完成状态,  SC039 测试"
                                         + " FROM SCHEDULE "
                                         + " LEFT JOIN BoxSizeCode ON SC036 = BoxCode "
                                         + " LEFT JOIN SplitTypeCode ON TypeCode = SC037 "
                                         + " WHERE 1=1  AND SC039 = 'Y' ";
            if (show)
            {
