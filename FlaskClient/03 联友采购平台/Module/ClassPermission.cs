...
namespace HarveyZ
{
    class UserPermission
    {
        ...
        public static void SetPermBase(List<string> getList)
        {
            ...
            {
                mssql.SQLexcute(FormLogin.infObj.connWG, sqlstrReset);

                foreach (string MenuListTmp in getList)
                {
                    if (mssql.SQLexist(FormLogin.infObj.connWG, string.Format(sqlstrFind, MenuListTmp)))
                    {
                        mssql.SQLexcute(FormLogin.infObj.connWG, string.Format(sqlstrSet, MenuListTmp));
                    }
                    else
                    {
                        mssql.SQLexcute(FormLogin.infObj.connWG, string.Format(sqlstrNew, MenuListTmp));
                    }
                }
            }
        }
        ...
        public static List<string> GetPermUser(string U_ID)
        {
            ...
            DataTable dt = mssql.SQLselect(FormLogin.infObj.connWG, string.Format(sqlstr, U_ID));
            if (dt != null)
            {
        ...
        public static void SetPermUser(string U_ID, List<string> getList)
        {
            ...
            if(getList != null && getList.Count != 0)
            {
                mssql.SQLexcute(FormLogin.infObj.connWG, string.Format(sqlstrDel, U_ID));
                foreach (string getListTmp in getList)
                {
                    mssql.SQLexcute(FormLogin.infObj.connWG, string.Format(sqlstrNew, U_ID, getListTmp));
                }
            }
        }
        #endregion
